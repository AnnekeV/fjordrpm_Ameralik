...
[z_sorted, idx] = sort(f.zs);          % ascending
Ss_sorted = f.Ss(idx,:);                 % apply same ordering
Ts_sorted = f.Ts(idx,:);


Sinterp = griddedInterpolant(z_sorted,Ss_sorted,'pchip','nearest');
S0 = Sinterp(z0);
Tinterp = griddedInterpolant(z_sorted,Ts_sorted,'pchip','nearest');
T0 = Tinterp(z0);
